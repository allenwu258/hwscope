...
src/
  ...
  HwScope.Core.Tests/
    Core 单元测试，当前覆盖 NVMe/ATA parser、健康判定、storage descriptor 边界和报告格式化
...
- `MSFT_Partition` / `MSFT_Volume` 物理磁盘、GPT/MBR 分区、无盘符卷和盘符映射。
...
- ATA SMART attributes/threshold parser、保守健康判定和 Windows SMART IOCTL provider。
- 设备级异步缓存、刷新、切换 stale-result guard、复制和保存 `.txt` 报告。
- CLI：
...
本机 NVMe 路径已使用真实 Samsung PM9F1 验证。ATA provider 已有 fixture tests，但仍需要在直连 SATA SSD/HDD 上完成硬件矩阵验证。USB bridge、RAID、Storage Spaces 和虚拟磁盘可能只能展示身份/卷信息，并明确显示 health passthrough 不可用。
