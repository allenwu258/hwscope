...
- `HwScope.Native.StorageBench` 已实现 create-new 文件准备、设备/系统缓存模式、IOCP overlapped I/O、四行 Read/Write/Mix、每轮统计、协议 v1、协作取消和 cleanup。
- Core runner 已实现目标卷级 lock file、10 分钟默认 timeout、2 秒 cooperative cancel grace、process-tree kill、父进程兜底删除和脱敏诊断日志。
- `%LOCALAPPDATA%\HwScope\StorageBench\Manifests` 保存 active session；窗口会发现残留并只清理通过 GUID/path/root/size/reparse-point 复核的文件。
- `StorageBenchmarkWindow` 已接入工具菜单、顶部工具栏和 `性能测试 -> 存储跑分`，包含参数、目标/预算条、四行三列矩阵、进度、取消、Diagnostics、Copy 和 TXT/JSON Save。
- CLI 已支持 `benchmark storage`、target/quick/size/runs/workload/JSON 参数；`--cancel-after-ms` 用于取消路径诊断。
...
仍未完成：durable/write-through 模式、后台 I/O 可靠检测、BitLocker/电源 quality flag、温度在未缓存详情时的主动物理盘关联、Storage Spaces/RAID、USB/SATA/HDD 广覆盖和长期 sustained/steady-state workload。当前结果不能直接等同于 CrystalDiskMark。
...
- 每个 sample 或 pass 改变 data seed，降低控制器压缩/去重造成的虚高。
- seed 写入 result，允许复现，不在 UI 主表展示。
...
每次运行在应用数据目录保存 session manifest，并在目标目录保存最小标记：

```text
sessionId
createdBy / workerVersion / protocolVersion
targetVolumeId
testFilePath
plannedFileSize
createdAt
...
manifest 不保存完整设备序列号。它用于父进程清理和下次启动发现孤儿文件，不能作为信任任意路径并删除的授权。
...
- 删除前重新验证文件名、目录、session ID、volume ID、文件 owner marker 和大小边界。
...
运行中 worker 维护实际 logical bytes read/written；超过 budget 前停止提交新 I/O。退出结果同时保存 logical bytes、OS acknowledged bytes 和 cleanup 状态。SSD NAND 实际写放大无法由应用准确得知，不应伪装为可测值。
...
stdout 使用 newline-delimited JSON。每条事件包含 `protocol_version`、`session_id`、单调递增 `sequence` 和 `type`。
...
- event sequence 缺失、倒退、跨 session 或 result/complete 缺失都视为 protocol error。
...
### Stage 3: Mix And Broader Targets

- Mix schedule 与 read/write split。
- USB removal、exFAT/ReFS、BitLocker 和更多物理设备验证。
- CLI `benchmark storage`。
...
- target identity 在执行前后核对，卷变化时不会把结果关联到错误物理盘。
...
进入完整开发方案前仍需通过原型或硬件验证确定：

- Windows overlapped I/O engine 使用 IOCP 还是每线程 event completion；外部 contract 不依赖这一选择。
