...
- 建立按需加载的 PCIe/USB 强类型拓扑、共享快照、深层 USB descriptor 隔离和精确诊断界面。
- 建立可扩展的主题配置系统。
...
src/HwScope.UsbWorker
  isolated .NET worker for deep USB descriptor queries

src/HwScope.Core.Tests
  Core parser、native buffer boundary、topology builder、cache/worker、health、benchmark 和 formatter tests

src/HwScope.App.Tests
  PCIe/USB diagnostic projection、filter、search、expansion 和 formatter tests
```
...
- `Pages/DeviceTopologyPage.xaml`
  `总线与端口` 页面。PCI Express/USB 使用独立标签和紧凑可展开拓扑投影，选择节点后在下方显示分组详情，并可打开对应精确诊断窗口。

- `Windows/DeviceTopologyDiagnosticsWindow.xaml`
  PCIe 单实例精确诊断窗口，展示 Root/Bridge/Endpoint/Function 目录、结构化属性和原始报告。

- `Windows/UsbTopologyDiagnosticsWindow.xaml`
  USB 单实例精确诊断窗口，展示 Controller/Hub/Port/Device 目录、空端口、按需 descriptor 和原始报告。
...
- `Services/DeviceTopologyService.cs`
  分别持有最新 PCIe/USB 不可变快照，合并同类并发采集，将结果发布回 WPF Dispatcher，并在刷新失败时通过 refresh policy 保留最近成功快照。USB 深层详情由其共享 `UsbDeviceDetailCache` 管理。
...
- `HwScope.Core.Hardware.Graphics`
  显卡 WMI/DXGI 快照合并。DXGI 提供专用显存、专用系统内存、共享系统内存与 LUID；PCI vendor/device/subsystem/revision 必须构成双向唯一匹配才补入 WMI 记录。相同型号多卡无法唯一关联时保留独立 DXGI 记录及诊断，不按名称、厂商或枚举顺序猜测。`AdapterRam` 保留 WMI 原始值供诊断，不能作为已确认的专用显存。
...
- `HwScope.Core.Hardware.DeviceTopology`
  PCIe/USB 共用的字段来源、availability、诊断和 PnP identity 模型。`Pci` 子命名空间负责 PCI 地址解析、SetupAPI 记录、树构建、刷新策略、脱敏和报告；`Usb` 子命名空间负责物理端口模型、树构建、刷新策略、深层详情、缓存、worker 协议、脱敏和报告。

- `HwScope.Core.Windows.Devices`
  通用只读 SetupAPI device/interface 枚举、property buffer 读取和 typed property value 转换。PCIe/USB domain source 只声明需要的 property keys 并消费 managed 结果，不直接定义 P/Invoke。

- `HwScope.Core.Windows.Usb`
  只读 USB hub handle/IOCTL 边界与 bounded native buffer/descriptor parser。Controller/Root Hub/Port 枚举和深层 descriptor 查询在此转换为 typed records，App 不接触 native buffer。
...
- 硬件采集和结果模型放 Core。
- GUI 只负责展示、交互和窗口生命周期。
- 主页面、PCIe 诊断窗口和 USB 诊断窗口消费同一个 `DeviceTopologyService`；窗口不各自创建采集器。
...
- USB 深层 descriptor 同样通过 `HwScope.UsbWorker` 隔离；请求有硬超时，超时会终止进程树，异常 descriptor 在 Core parser 边界内降级为诊断。
...
`HwScope.UsbWorker` 是 `net8.0-windows` 单请求进程，用于隔离 USB configuration/interface/endpoint/string/BOS descriptor 的深层读取。App/Core 通过 stdin 发送一个 JSON target，worker 调用 Core 的只读 USB collector，并通过 stdout 返回一个 `UsbDeviceDetailSnapshot`。

`UsbDeviceDetailCache` 以 physical attachment ID 为键，合并同一设备的并发加载，限制并发 worker 数，并在新的 USB topology snapshot 表明设备断开或 attachment 变化时清理缓存。worker 默认有 12 秒硬超时；缺失、超时、非零退出或无效 JSON 都作为诊断/错误处理，不允许阻塞 UI 线程或污染其他设备缓存。
...
3. App 创建 `ThemeService`、`SingleInstanceWindowManager`、`HardwarePreloadService`、`StorageDetailService` 和按需加载 PCIe/USB 的 `DeviceTopologyService`。
...
`DeviceTopologyService` 不参与启动 preload。首次进入 `总线与端口` 页面时只加载 PCIe；切换到 USB 标签或打开 USB 诊断窗口时才枚举 USB 物理端口。深层 USB descriptor 只在选择适用节点后通过 worker 加载。
...
- 为硬件摘要补更细的主板、显卡详情页。
- 为 PCIe/USB 拓扑接入 device notification、snapshot diff、完整 PCIe resources、USB composite function 子节点、PCIe/USB controller 跨树跳转、跨存储/网络/GPU 定位和默认脱敏的 full snapshot JSON，并扩展真实硬件验证矩阵。
