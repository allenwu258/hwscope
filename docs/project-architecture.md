...
SPD worker 当前通过 CMake 构建 `spd.exe`，并提供 Memory / SPD 详情页后续 raw SPD 读取的进程边界：
...
1. `App.OnStartup` 创建 `ThemeService`、`SingleInstanceWindowManager` 和 `HardwarePreloadService`。
2. App 显示 `HardwarePreloadWindow`，窗口在 `Loaded` 后接入 `ThemeService`。
3. `HardwarePreloadWindow` 调用 `App.HardwarePreload.RefreshAsync()`，显示步骤进度。
4. 预加载成功后创建并显示 `MainWindow`，再关闭预加载窗口。
5. 预加载失败时，用户可以重试，或继续进入主窗口并在后续页面刷新时重试。
...
- 为内存 / SPD 详情补 raw SPD reader、JEDEC/XMP/EXPO profile、DDR5 组织字段和运行态时序 provider。
- 为硬件摘要补更细的主板、显卡详情页。
