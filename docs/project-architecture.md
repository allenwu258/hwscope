...
- 接入 native SPD offline parser worker。
- 建立可扩展的主题配置系统。
...
src/HwScope.Native.Spd
  native C++ SPD fixture/offline parser worker
```
...
SPD worker 当前通过 CMake 构建 `spd.exe`，提供 fixture/offline raw bytes 解析和 Memory / SPD 页面的 JSON 协议边界：

```text
spd.exe --json
```

Core 的 `NativeSpdProcessProvider` 会按以下顺序查找 worker：
...
默认 `auto` backend 返回 schemaVersion 1、workerVersion、`notImplemented` 状态、空 modules 和 diagnostics，页面显示 `SPD 读取暂未实现`。`fixture` backend 支持 payload fixture 和 raw `bytesHex` parser，可输出 CRC/SHA、组织、电压和 timing fields。Windows SMBus probe 和真实硬件 reader 已移除；内核驱动相关能力处于 parked 状态。`HwScope.App` 和 `HwScope.Cli` 在构建后会在 native Release 产物存在时复制到各自输出目录的 `native\spd.exe`。

## GUI Startup And Preload

当前 GUI 启动顺序：

1. `App.OnStartup` 尝试通过 UAC 启动管理员权限实例。
2. 用户取消或提权失败时显示可关闭警告，并继续以普通权限运行。
3. `App.OnStartup` 创建 `ThemeService`、`SingleInstanceWindowManager` 和 `HardwarePreloadService`。
...
- 继续稳定启动预加载窗口、内存跑分窗口打开、主题附着和窗口生命周期。
- 将 native worker 构建纳入更完整的开发/发布流水线。
- 为主题系统补配置迁移和用户自定义主题枚举。
- 为 CPU 详情补 native CPUID worker、真实 feature flags 和 L3/CCD 洞察。
- 继续完善 offline SPD parser、JEDEC/XMP/EXPO profile 和 DDR5 raw bytes 解码；raw hardware reader、运行态时序和 telemetry 保持 parked，等待独立驱动方案。
- 为硬件摘要补更细的主板、显卡详情页。
